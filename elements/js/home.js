...
$(document).ready(() => {
    ...
    let postfunc = (data) => {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            let $box = $("<div class='post '>").html(
                ' <div class="partentIMG"> <img src="' + post.miniature + ' " ></div> <h3 class="text-center">' +
                post.name +
                ...
                post.prix +
                post.monnais +
                " </strong> </p><pan id='PPD-" + post.id + "'>   " +
                dispo(post.disponible) +
                '</span>  <p class="date" > Publié le :  <time datetime="' + time(post.p_date, "-") + '"> ' +
                time(post.p_date) +
            ...
            if ((window.location.href).includes("/admin")) {
                ...
                let $admin = $('<span>').html('<hr> <strong id="idPostNumber">ID:  ' + post.id + '</p> </strong> <hr>   <a href="#DeleteObj" id="DeletPost-' + post.id + '" class = "btn text-danger " >  Supprimer </a >  <a href="#EditPost" id="EditPost-' + post.id + '"  class = "btn text-primary"> ' + $(dispo(post.disponible)).text() + ' </a>   ')
                $admin.addClass('my-3')
    ...
    ajax({
        ...
        (data) => {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                let $box = $("<li>").html(' <a href="#' + categ.slug + '" id="' + categ.id + ' "   > ' + categ.name + "</a>");

                if ((window.location.href).includes("/admin")) {

                    let $admin = $('<span>').html(' <p >SLUG:  ' + categ.slug + '</p>  <a href="#DeleteObj" id="DeletCtg-' + categ.id + '" class = " btn text-danger " > SUpprimer </a><a href = "#EditCategorie" id = "EditCtg-' + categ.id + '"  class = "btn text-primary"> Editer </a> <hr>')
    ...
    setTimeout(() => {
        ...
        if (!(window.location.href).includes("/admin") && (window.location.href).includes("#")) {
